...
2024-10-07: 'upgraded' to `block` methodology - MyHDL 0.11

__updated__="2024-10-07 12:27"
...
from myhdl import (block, Signal, intbv, always_seq, always_comb, concat, enum,
                   instances, instance, StopSimulation, ResetSignal)
import hdlutils
...
def KalmanFilter(WIDTH_DQ, WIDTH_COEFF, Clk, Reset, D, StrobeIn, Q, StrobeOut, CoeffA, CoeffB):
    ...
    def smcomb():
        ...
        shiftshift.next = 0
        ldyn.next = 0
        # These default assignments save LUTs
    ...
    def smsync():
        ''' the synchronous part of the state machine has a reset '''
        smkfp.next = smkfn

    @always_seq(Clk.posedge, reset=None)
    def smreg():
        ''' the registered (or dataflow) part of the state machine dopesn't need a reset '''
...
def tb_KalmanFilter():
    ''' simulate '''
    dut = KalmanFilter(WIDTH_DQ, WIDTH_COEFF, Clk, Reset, D, StrobeIn, Q, StrobeOut, CoeffA, CoeffB)
...
if __name__ == '__main__':
    ...
    def convert():
        dfc = KalmanFilter(WIDTH_DQ, WIDTH_COEFF, Clk, Reset, D, StrobeIn, Q, StrobeOut, CoeffA, CoeffB)
        # force std_logic_vectors instead of unsigned in Interface
        dfc.convert(hdl='VHDL', std_logic_ports=True)
        dfc.convert(hdl='Verilog', no_testbench=True)

    def runtestbench():
        dft = tb_KalmanFilter()
        dft.config_sim(trace=True, tracebackup=False)
        dft.run_sim()
